...
# File "activity.csv" needs to be placed in the same directory as this Rmarkdown file  
...
```{r part2}
# Tidy Data -> Ignore missing values from the data set
...
``` {r part3}
## Create time series plot of the 5-minute interval & the average number of steps taken
...
naInterval = data[naIdx, 3] # Get corresponding interval
fillSteps = sapply(naInterval, function(x) { avgStep[(avgStep$interval==x), 2]})

# Create a new dataset that is equal to the orginal dataset, with missing data populated
dataNew = data
dataNew[naIdx, 'steps'] = fillSteps # Populate missing data
...
# Calculate average of steps w.r.t. time interval and dateIs
